...
type Platillo = {
  ...
  descripcion: string;
  precio: string;
  imagen?: string; // si en el futuro agregas imagen desde DB
};
...
const FuertesScreen = () => {
  ...
  const categoriaId = "672ae9d180bcc4b66bcc10a6"; // <- Reemplaza esto por el ID real
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollContainer}>
        <Text style={styles.titulo}>Platos Fuertes</Text>

        {platillos.map((plato) => (
          <View key={plato._id} style={styles.card}>
            {/* Aquí puedes usar una imagen fija mientras no haya URL en la DB */}
            <Image source={require('../../assets/fuerte.png')} style={styles.imagen} />
...
const styles = StyleSheet.create({
  ...
  descripcion: { fontSize: 16, color: "#666", marginBottom: 12, lineHeight: 22 },
  precioContainer: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  precio: { fontSize: 20, fontWeight: "bold", color: "#e74c3c" },
  botonAñadir: { backgroundColor: "#27ae60", paddingVertical: 10, paddingHorizontal: 20, borderRadius: 25 },
